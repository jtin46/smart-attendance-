...
export default function AdminDashboard({ user }: AdminDashboardProps) {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto p-4 md:p-8" id="admin-dashboard-view">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="bg-indigo-50 border border-indigo-100 px-4 py-3 rounded-xl flex items-center gap-2.5">
          <div className="bg-indigo-500 p-2 rounded-lg text-white">
            <School className="h-5 w-5" />
          </div>
          <div>
            <div className="text-xs text-indigo-400 font-medium">Total Campuses</div>
            <div className="text-lg font-bold text-indigo-900">{colleges.length}</div>
          </div>
        </div>
      </div>
      ...
      {/* Tabs navigation */}
      <div className="flex border-b border-slate-200 gap-6" id="admin-tabs">
        <button
          onClick={() => { setActiveTab("colleges"); setSuccessMsg(null); }}
          className={`pb-4 text-sm font-semibold transition-all duration-200 border-b-2 flex items-center gap-2 ${
            activeTab === "colleges" ? "border-indigo-600 text-indigo-600" : "border-transparent text-slate-400 hover:text-slate-800"
          }`}
        >
          <School className="h-4 w-4" /> Create College
        </button>
        <button
          onClick={() => { setActiveTab("structure"); setSuccessMsg(null); }}
          className={`pb-4 text-sm font-semibold transition-all duration-200 border-b-2 flex items-center gap-2 ${
            activeTab === "structure" ? "border-indigo-600 text-indigo-600" : "border-transparent text-slate-400 hover:text-slate-800"
          }`}
        >
          <Sliders className="h-4 w-4" /> College Structure
        </button>
        <button
          onClick={() => { setActiveTab("users"); setSuccessMsg(null); }}
          className={`pb-4 text-sm font-semibold transition-all duration-200 border-b-2 flex items-center gap-2 ${
            activeTab === "users" ? "border-indigo-600 text-indigo-600" : "border-transparent text-slate-400 hover:text-slate-800"
          }`}
        >
        ...
        <button
          ...
          className={`pb-4 text-sm font-semibold transition-all duration-200 border-b-2 flex items-center gap-2 ${
            activeTab === "support" ? "border-indigo-600 text-indigo-600" : "border-transparent text-slate-400 hover:text-slate-800"
          }`}
